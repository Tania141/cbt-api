...
def build_placeholders(d):
    ...
    return {
        ...
        **({
        ...
        # Съдържанието на раздели В и Г — попълва се от слой 2 (AI чете записките).
        # Незапълнена точка остава [___], за да личи, че липсва — не празно.
        # Правилото е от промпта на оператора: липсващо данно НЕ се измисля.
        "{{ОСИП_В_ПУП}}":                d.get("ОСИП_В_ПУП","").strip() or "[___]",
        "{{ОСИП_В_Части}}":              d.get("ОСИП_В_Части","").strip() or "[___]",
        "{{ОСИП_Г_ПУП}}":                d.get("ОСИП_Г_ПУП","").strip() or "[___]",
        "{{ОСИП_Г_Норми}}":              d.get("ОСИП_Г_Норми","").strip() or "[___]",
        **{f"{{{{ОСИП_Г_{code}}}}}": d.get(f"ОСИП_Г_{code}","").strip() or "[___]"
