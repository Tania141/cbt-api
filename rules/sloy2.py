    ...
    return {
        ...
        "properties": {
            ...
            "material": _fakt("САМО при декларация/сертификат за строителен продукт: материалът или "
                              "изделието, буквално — напр. „Обикновен бетон“, „кабел тип СВТ 0.6/1kV“"),
            "proizvoditel": _fakt("САМО при декларация/сертификат за строителен продукт: производителят "
                                  "или доставчикът, буквално — напр. „АТМИКС ООД“"),
            ...
            "nt_notarius": _fakt("САМО при нотариален акт: името на нотариуса, буквално (без номера му)"),
            "nt_nk": _fakt("САМО при нотариален акт: номерът на нотариуса в регистъра на Нотариалната камара — "
                           "напр. „701“ от „вписан в регистъра на НК под № 701“"),
...
- При нотариален акт попълни данните на акта (nomer, data, nt_tom, nt_reg, nt_delo, nt_notarius — името на нотариуса, nt_nk — номерът му в регистъра на Нотариалната камара), вписването в Службата по вписванията (vp_vh_reg, vp_akt, vp_tom, vp_delo — обикновено в печата на вписването) и „razpredelenie“ — кои обекти (апартаменти, гаражи, паркоместа, ателиета) на кой собственик остават. Всичко буквално.
...
def _normalizirai(dok):
    ...
    for k in ("nt_tom", "nt_reg", "nt_delo", "nt_notarius", "nt_nk", "vp_vh_reg", "vp_akt", "vp_tom", "vp_delo"):
        if k in dok:
            dok[k] = fakt(dok[k])
    dok["razpredelenie"] = [
