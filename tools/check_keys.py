...
#
# Двете предпазни части не са украса:
#   (?<!\w)   — иначе всяко име, което свършва на „d“, минава за речника `d`.
#               `red["status"] == "заменена"` се броеше за вписване (09.09.2026).
#   (?!=)     — иначе сравнението `==` се брои за присвояване.
ENDPOINT_SET = re.compile(r'(?<!\w)d\[\s*"([^"]+)"\s*\]\s*=(?!=)')
...
OPTIONAL = {
    ...
    "Акт12_Номер",
    # Остава за ръчно попълване НАРОЧНО (09.09.2026): кой орган назначава
    # комисията се мени със заповед на началника на ДНСК и надзорът го знае
    # по-добре от всяка таблица, зашита тук. Виж коментара в cbt_docx.py.
    "ДНСК_Орган",
}
